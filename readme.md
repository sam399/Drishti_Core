...
* **Semantic Language Augmentation & Foveated Lens:** Fully integrated in the backend (`server.py`) and frontend (`viewer.html`). Uses a local `LaBSE` dense encoder and in-memory `Qdrant` vector database client to provide real-time context-aware Bengali sentence suggestions from a conversational/emergency corpus.
